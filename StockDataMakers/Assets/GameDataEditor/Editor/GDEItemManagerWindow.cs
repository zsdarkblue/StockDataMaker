        ...
        void DrawEntry(string schemaType, string itemKey, Dictionary<string, object> data)
        {
            ...
            if (isOpen)
            {
                ...
                foreach(BasicFieldType fieldType in GDEItemManager.BasicFieldTypes)
                {
                    ...
                    if (!runtimeValues)
                        fieldKeys = GDEItemManager.ItemFieldKeysOfType(itemKey, fieldType.ToString(), 0);
                    else
                        fieldKeys = GDEItemManager.ItemFieldKeysOfType(itemKey, fieldType.ToString(), entriesToDraw, 0);
